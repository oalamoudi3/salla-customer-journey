...
async function refreshIfNeeded(blobs, storeKey, cfg) {
  ...
  // No refresh needed/possible — make sure the seed is persisted so the auth blob
  // exists and request-path functions can read it.
  if (!rec.refreshedAt) await blobs.setJSON(storeKey, { ...rec, lockedAt: 0 });
  return rec.access_token || "";
}
