...
function failedPaymentsSummary(txs, cfg) {
  ...
  // DEDUPE FOR DISPLAY: one row per customer (a single customer often fails repeatedly —
  // e.g. 4 Tamara attempts). We keep every attempt in the totals, but the table shows each
  // customer once with an `attempts` count and their TOTAL failed value + latest method/date.
  const byCust = {};
  for (const r of rows) {
    const key = r.customerId || ("tx_" + r.txId);
    const g = byCust[key] || (byCust[key] = { customerId: r.customerId, name: r.name, sum: 0, attempts: 0, _latestMs: 0, txId: r.txId, method: r.method, card: r.card, orderId: r.orderId, createdAt: r.createdAt });
    g.sum += r.amount; g.attempts++;
    if (!g.name && r.name) g.name = r.name;
    if (r.createdMs >= g._latestMs) { g._latestMs = r.createdMs; g.txId = r.txId; g.method = r.method; g.card = r.card; g.orderId = r.orderId; g.createdAt = r.createdAt; }
  }
  // `value` = AVERAGE attempt amount (= the order value they were trying to pay), NOT the sum
  // of retries — so 3 attempts of 5,000 reads as 5,000, not 15,000.
  const custs = Object.values(byCust).map(g => ({ ...g, value: g.attempts ? Math.round(g.sum / g.attempts) : 0 }));
  const top = custs.sort((a, b) => b.value - a.value).slice(0, cfg.PULL.failedTopN || 10)
    .map(g => ({ txId: g.txId, customerId: g.customerId, name: g.name, value: g.value, attempts: g.attempts, method: g.method, card: g.card, orderId: g.orderId, createdAt: g.createdAt }));
  return {
    count: rows.length,                       // total failed attempts (all)
    customers: custs.length,                  // distinct failing customers
    value: custs.reduce((s, c) => s + c.value, 0), // sum of per-customer averages = realistic at-risk value
    top
  };
}
...
async function fetchCustomerAdminUrls(token, ids, cfg) {
  ...
  for (const id of uniq) {
    for (let attempt = 0; attempt < 5; attempt++) {     // 429-aware (these run amid the heavy refresh)
      try {
        const res = await fetch(`${SALLA_BASE}/customers/${id}`, { headers: { Authorization: `Bearer ${token}`, Accept: "application/json" } });
        if (res.status === 429) { const ra = Number(res.headers.get("retry-after")); await sleep(ra > 0 ? ra * 1000 : Math.min(1500 * 2 ** attempt, 12000)); continue; }
...
/* #4 — cross-store journey. storeCusts = { storeKey: [{h(ash), id, name, revenue}] } for the
   stores that were refreshed. A customer's CURRENT stage = the furthest store (in
   CROSS_JOURNEY.order) they've purchased from; the recommendation = the next store they
   haven't bought from yet. Returns, professionally and ACTIONABLY:
     - stages[]       : per stage — reach count, "furthest here" count, value at the stage
     - conversions[]  : stage→next conversion RATE (of those who reached a stage, how many moved on)
     - recommend{}    : per next-stage — how many to push there, their combined value, and an
                        actionable SAMPLE (source-store customer id+name+revenue → viewable/contactable)
   No raw PII: candidate rows carry only the source-store customer id + name + revenue (same as the
   index); the cross-store join key is a one-way mobile hash that is never emitted. */
function computeJourney(storeCusts, cfg) {
  const order = ((cfg.CROSS_JOURNEY || {}).order || Object.keys(cfg.STORES || {})).filter(k => storeCusts[k]);
  const idx = {}; // hash -> { storeKey: {id,name,revenue} }
  order.forEach(k => storeCusts[k].forEach(c => { if (c && c.h) (idx[c.h] = idx[c.h] || {})[k] = c; }));
  const perStore = {}; order.forEach(k => perStore[k] = 0);
  const stageDist = {}, stageValue = {}, recommend = {}, recSamples = {};
  let total = 0, multi = 0, multiValue = 0;
  for (const h in idx) {
    total++;
    const rec = idx[h];
    const present = order.filter(k => rec[k]);
    if (present.length > 1) { multi++; multiValue += present.reduce((s, k) => s + (rec[k].revenue || 0), 0); }
    let furthestIdx = -1;
    order.forEach((k, i) => { if (rec[k]) { perStore[k]++; furthestIdx = i; } });
    const furthest = order[furthestIdx];
    stageDist[furthest] = (stageDist[furthest] || 0) + 1;
    stageValue[furthest] = (stageValue[furthest] || 0) + (rec[furthest].revenue || 0);
    const next = order[furthestIdx + 1];
    if (next && !rec[next]) {
      recommend[next] = (recommend[next] || 0) + 1;
      const src = rec[furthest]; // their record in the store they last reached → where the team acts
      (recSamples[next] = recSamples[next] || []).push({ store: furthest, id: src.id, name: src.name, revenue: src.revenue || 0 });
    }
  }
  const conversions = [];
  for (let i = 0; i < order.length - 1; i++) {
    const from = order[i], to = order[i + 1];
    let fromCount = 0, both = 0;
    for (const h in idx) { if (idx[h][from]) { fromCount++; if (idx[h][to]) both++; } }
    conversions.push({ from, to, fromCount, both, rate: fromCount ? Math.round(both / fromCount * 100) : 0 });
  }
  const N = (cfg.CROSS_JOURNEY || {}).candidateSample || 12;
  const rec = {};
  for (const next in recommend) {
    const s = (recSamples[next] || []).sort((a, b) => b.revenue - a.revenue);
    rec[next] = { count: recommend[next], value: Math.round(s.reduce((x, c) => x + c.revenue, 0)), sample: s.slice(0, N) };
  }
  const stages = order.map(k => ({ store: k, reach: perStore[k], furthest: stageDist[k] || 0, furthestValue: Math.round(stageValue[k] || 0) }));
  return { order, totalMatched: total, multiStore: multi, multiStoreValue: Math.round(multiValue), stages, conversions, recommend: rec, perStore, stageDist, generatedAt: Date.now() };
}
