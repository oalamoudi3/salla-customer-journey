...
/* Generic paged GET with the same 429-aware retry + pacing as orders. Used for the
   abandoned-carts and products pulls. */
async function fetchAllPaged(token, path, perPage, maxPages) {
...
/* #3 — abandoned/failed carts (did not become orders). Non-critical: any failure returns
   an empty summary rather than breaking the whole refresh. */
async function fetchAllAbandoned(token, cfg) {
  try { return await fetchAllPaged(token, "carts/abandoned", cfg.PULL.perPage, cfg.PULL.cartsMaxPages || 40); }
  catch (e) { return []; }
}
...
async function fetchProductMap(token, cfg) {
  try {
    const prods = await fetchAllPaged(token, "products", cfg.PULL.perPage, cfg.PULL.productsMaxPages || 120);
    const map = {};
...
function abandonedSummary(carts, cfg) {
  ...
  const rows = carts.map(c => {
    let city = readField(c, FM.city); if (city && typeof city === "object") city = city.name || city.city || "";
    ...
    return {
      ...
      items, ageMinutes: Math.round(Number(readField(c, FM.ageMinutes)) || 0)
    };
  }).filter(r => r.total > 0);
  return {
    count: rows.length,
...
async function buildSegments({ storeKey, token, cfg, debug }) {
  ...
  if (!token) {
    ...
    return Object.assign({ source: "sample", returnedOrders: series.returnedTotal || 0, abandoned: SEG.genSampleAbandoned(storeKey, cfg) }, meta, agg, { byType }, series);
  }
  ...
  const prodMap = await fetchProductMap(token, cfg);             // #2 — name→category map
  const { agg, byType, allCustomers } = segmentByCohort(rollup(orders, cfg), cfg, storeKey);
  const series = timeSeries(orders, cfg, prodMap);
...
module.exports = { buildSegments, rollup, timeSeries, classifyOrder, statusSets, computeJourney };
