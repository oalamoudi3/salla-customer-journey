...
function failedPaymentsSummary(txs, cfg) {
  ...
  for (const r of rows) {
    ...
    const g = byCust[key] || (byCust[key] = { customerId: r.customerId, name: r.name, value: 0, attempts: 0, _latestMs: 0, txId: r.txId, method: r.method, card: r.card, orderId: r.orderId, createdAt: r.createdAt });
    g.value += r.amount; g.attempts++;
  ...
  const top = Object.values(byCust).sort((a, b) => b.value - a.value).slice(0, cfg.PULL.failedTopN || 10)
  ...
  return {
    ...
    customers: Object.keys(byCust).length,    // distinct failing customers
