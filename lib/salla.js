...
/* orders → time-bucketed trend series (month/quarter/year) + per-period and all-time
   ...
   Output: { monthly, geo, periods:{month,quarter,year}, returnedTotal } */
function timeSeries(orders, cfg) {
  ...
  const custReg = {}, valid = [], returnedEvents = [];
  for (const o of orders) {
...
async function buildSegments({ storeKey, token, cfg, debug }) {
  ...
  if (!token) {
    ...
    return Object.assign({ source: "sample", returnedOrders: series.returnedTotal || 0 }, meta, agg, { byType }, series);
  }
