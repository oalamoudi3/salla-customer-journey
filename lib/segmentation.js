...
(function (root, factory) {
  ...
  function genSampleAbandoned(storeKey, cfg) {
    ...
    const midAov = s.aov ? (s.aov[0] + s.aov[1]) / 2 : 400;
    const cities = ["الرياض", "جدة", "الدمام", "مكة المكرمة", "الخبر", "أبها"];
    ...
    for (let i = 0; i < n; i++) {
      ...
      top.push({ total, city: cities[Math.floor(rnd() * cities.length)], name: rnd() < 0.5 ? "" : names[Math.floor(rnd() * names.length)], items: 1 + Math.floor(rnd() * 6), ageMinutes: Math.floor(rnd() * 4000) });
    }
  ...
  return { thresholds, scoreStore, aggregate, segmentOf, lifecycleOf, statusOf, validVat, customerTypeOf, genSampleCustomers, genSampleSeries, genSampleAbandoned };
});
