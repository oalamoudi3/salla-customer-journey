...
(function (root, factory) {
  ...
  /* deterministic sample monthly trend + geo breakdown (demo/offline mode only) */
  function genSampleSeries(storeKey, cfg) {
    ...
    for (let i = nMonths - 1; i >= 0; i--) {
      ...
      monthly.push({ month, revenue, orders, aov, buyers: Math.round(orders * (0.7 + rnd() * 0.25)), newCustomers: Math.round(orders * (0.12 + rnd() * 0.2)) });
    }
    const cities = ["الرياض", "جدة", "الدمام", "مكة المكرمة", "المدينة المنورة", "الخبر", "أبها", "تبوك", "بريدة", "الطائف"];
    const geo = cities.map((city, i) => {
      const w = (cities.length - i) * (0.6 + rnd() * 0.9);
      const orders = Math.round(40 * w);
      return { city, revenue: Math.round(orders * midAov), orders, customers: Math.round(orders * (0.55 + rnd() * 0.3)) };
    }).sort((a, b) => b.revenue - a.revenue);
    return { monthly, geo };
  }

  return { thresholds, scoreStore, aggregate, segmentOf, lifecycleOf, statusOf, genSampleCustomers, genSampleSeries };
});
