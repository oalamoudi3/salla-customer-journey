...
(function (root, factory) {
  ...
  function genSampleAbandoned(storeKey, cfg) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      top.push({ total, customerId: String(700000 + Math.floor(rnd() * 99999)), name: names[Math.floor(rnd() * names.length)], checkoutUrl: "#", items: 1 + Math.floor(rnd() * 6), ageMinutes: Math.floor(rnd() * maxAgeMin) });
    }
