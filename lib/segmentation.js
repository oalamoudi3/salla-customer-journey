  ...
  function genSampleFailed(storeKey, cfg) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      top.push({ txId: String(800000000 + Math.floor(rnd() * 99999999)), customerId: String(700000 + Math.floor(rnd() * 99999)),
        name: names[Math.floor(rnd() * names.length)], amount: Math.round(midAov * (1.2 + rnd() * rnd() * 7) * (1 - i * 0.03)),
        method: m[0], card: m[1], orderId: null, adminUrl: "#",
        createdAt: new Date(now - Math.floor(rnd() * 15 * 864e5)).toISOString().replace("T", " ").slice(0, 19) });
    }
    top.sort((a, b) => b.amount - a.amount);
    const count = Math.max(top.length, Math.round((s.n || 100) * (0.1 + rnd() * 0.25)));
    const value = top.reduce((x, r) => x + r.amount, 0) + Math.round(count * midAov * 0.5);
    return { count, value, top };
  }
