...
(function (root, factory) {
  ...
  function genSampleSeries(storeKey, cfg) {
    ...
    for (let i = nMonths - 1; i >= 0; i--) {
      ...
      monthSeries.push({ period, revenue, orders, aov, buyers: Math.round(orders * (0.7 + rnd() * 0.25)), newCustomers: Math.round(orders * (0.12 + rnd() * 0.2)) });
    ...
    const fold = (keyOf) => {
      ...
      monthSeries.forEach(m => {
        ...
        const p = map[k] || (map[k] = { period: k, revenue: 0, orders: 0, buyers: 0, newCustomers: 0 });
        p.revenue += m.revenue; p.orders += m.orders; p.buyers += m.buyers; p.newCustomers += m.newCustomers;
        const g = geo[k] || (geo[k] = {});
      ...
      const series = Object.values(map).map(p => ({ period: p.period, revenue: p.revenue, orders: p.orders,
        buyers: Math.round(p.buyers * 0.82), aov: p.orders ? Math.round(p.revenue / p.orders) : 0, newCustomers: p.newCustomers }))
    ...
    const quarter = fold(mk => { const p = mk.split("-"); return p[0] + "-Q" + (Math.floor((Number(p[1]) - 1) / 3) + 1); });
    const year = fold(mk => mk.split("-")[0]);
    const monthly = monthSeries.map(m => ({ month: m.period, revenue: m.revenue, orders: m.orders, buyers: m.buyers, aov: m.aov, newCustomers: m.newCustomers }));
    const allGeo = {};
    ...
    const geo = Object.values(allGeo).map(c => ({ city: c.city, revenue: c.revenue, orders: c.orders, customers: Math.round(c.customers * 0.6) }))
      .sort((a, b) => b.revenue - a.revenue).slice(0, topCities);
    return { monthly, geo, periods: { month: { series: monthSeries, geo: monthGeo }, quarter, year } };
  }

  return { thresholds, scoreStore, aggregate, segmentOf, lifecycleOf, statusOf, validVat, customerTypeOf, genSampleCustomers, genSampleSeries };
});
